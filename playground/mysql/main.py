...
from db.mysql import MyDB, init_db
...
def main():
    ...
    if (err := init_db(**db_config)) and err:
    ...
    if (
        err := my_db.create_table(
            tb_name=tb_countries,
            fields=list(
                [
                    f"{row_id} INT NOT NULL AUTO_INCREMENT",
                    f"{row_country} VARCHAR(255) NOT NULL",
                    f"{row_capital} VARCHAR(255) NOT NULL",
                    f"PRIMARY KEY ({row_id})",
                ]
            ),
        )
    ) and err:
    ...
    if (
        insert_row_err := my_db.insert_row(
            tb_name=tb_countries,
            rows=[row_country, row_capital],
            vals=("Ukraine", "Kyiv"),
        )
    ) and insert_row_err:
        die(f"INSERT_ROW_ERROR: {repr(insert_row_err)}")
    ...
    delete_count, delete_error = my_db.delete(
        tb_name=tb_countries, where=[row_country], vals=["India"]
    )
    if delete_error:
